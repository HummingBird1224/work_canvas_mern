...
import {getFeatureList} from '../../actions/action';
...
const FeatureList = () => {
  ...
  useEffect(()=>{
    async function getData(){
      companyId && await getFeatureList(companyId)
        .then(res=>{
          if(res.status == '200') {
    ...
    <div className="enterprise__container">
      <div className="enterprise__box ">
        ...
        {/* <ActionAlert
          text='確認が必要な質問があります。'
          className='mb-10'
          link={featureList.length>0&&featureList[0].id}
        /> */}
        <div className="d-flex flex-wrap justify-content-between">
          {featureList.length>0&&featureList.map((fList) => (
            <FeatureCard key={fList.id} props={fList} />
          ))}
        </div>
      </div>
    </div>
